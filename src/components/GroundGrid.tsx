...
export const GroundGrid: React.FC = () => {
  // 以底部均分 x，計算每條透視線
  const bottomXs = Array.from(
    { length: VERTICAL_LINE_COUNT },
    (_, i) => -16 + (152 * i) / (VERTICAL_LINE_COUNT - 1),
  );
  ...
  const horizonXs = bottomXs.map(
    (xb) => PERSPECTIVE_X + ((xb - PERSPECTIVE_X) * (HORIZON_Y - TOP_Y)) / (BOTTOM_Y - TOP_Y),
  );

  // 水平線 y 座標
  const horizontals = Array.from(
    { length: 6 },
    (_, i) => HORIZON_Y + ((BOTTOM_Y - HORIZON_Y) * i) / 5,
  );
  ...
  // ...
  const gridH = Array.from(
    { length: Math.floor((GRID_BOTTOM - GRID_TOP) / SQUARE_SIZE) + 1 },
    (_, i) => GRID_BOTTOM - (i + 1) * SQUARE_SIZE,
  );
